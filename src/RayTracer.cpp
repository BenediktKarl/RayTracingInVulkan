...
RayTracer::RayTracer(const UserSettings& userSettings, const Vulkan::WindowConfig& windowConfig,
                     const VkPresentModeKHR presentMode) : Application(windowConfig, presentMode,
...
void RayTracer::CreateSwapChain() {
    ...
    userInterface_.reset(new UserInterface(CommandPool(), SwapChain(), DepthBuffer(), userSettings_, userTelemetry_));
...
void RayTracer::Render(VkCommandBuffer commandBuffer, const uint32_t imageIndex) {
    ...
    // Render the UI
    static std::array<float, 100> occupancyHistory;
    ...
    auto* occupancy_values = static_cast<uint32_t*>(scene.OccupancyBufferMemory().Map(0, 2 * sizeof(uint32_t)));
    stats.OccupancyAmountSubgroups = occupancy_values[0];
    stats.OccupancyAmountActiveLanes = occupancy_values[1];
    float opc =
        static_cast<float>(stats.OccupancyAmountActiveLanes) /
        (static_cast<float>(rayTracingProperties_->SubgroupSize()) * static_cast<float>(stats.OccupancyAmountSubgroups));
    occupancyHistory[occupancyHistory.size() - 1] = opc;
    std::rotate(occupancyHistory.begin(),
                occupancyHistory.begin() + 1,
                occupancyHistory.end());
    stats.OccupancyPercentage.resize(occupancyHistory.size());
    std::copy(occupancyHistory.begin(), occupancyHistory.end(), stats.OccupancyPercentage.begin());

    occupancy_values[0] = 0;
    occupancy_values[1] = 0;
    scene.OccupancyBufferMemory().Unmap();
        ...
        if (!userSettings_.Benchmark) {
            switch (key) {
                ...
                case GLFW_KEY_H: userSettings_.ShowHeatmap = !userSettings_.ShowHeatmap;
                    break;
                case GLFW_KEY_P: isWireFrame_ = !isWireFrame_;
                    break;
                default: break;
            }
        }
...
void RayTracer::CheckAndUpdateBenchmarkState(double prevTime) {
    ...
    // Print out the frame rate at regular intervals.
    {
        ...
        if (periodTotalFrames_ != 0 && static_cast<uint64_t>(prevTotalTime / period) != static_cast<uint64_t>(
                totalTime / period)) {
    ...
    {
        const bool timeLimitReached = periodTotalFrames_ != 0 && Window().GetTime() - sceneInitialTime_ > userSettings_.
                                      BenchmarkMaxTime;
